...
class Metrics:
  def __init__(self, *_args, **_kwargs) -> None:
    ...
    # New metric: Denoising Log-Likelihood (calculated only on jumped tokens)
    metrics_denoising = torchmetrics.MetricCollection({
        'nll_denoising': NLL(), 'bpd_denoising': BPD(), 'ppl_denoising': Perplexity()})
    metrics_denoising.set_dtype(torch.float64)
    self.valid_nlls_denoising = metrics_denoising.clone(prefix='val/')
  ...
  def to(self, *args, **kwargs):
    ...
    self.valid_nlls_denoising = self.valid_nlls_denoising.to(*args, **kwargs)
  ...
  def reset(self):
    ...
    self.valid_nlls_denoising.reset()
  ...
  def update_valid_denoising(self, nll, num_tokens):
    self.valid_nlls_denoising.update(nll, num_tokens)
