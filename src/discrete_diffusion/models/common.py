"""Shared model primitives for GDDS backbones.
...
def split_and_apply_rotary_pos_emb(qkv: torch.Tensor, rotary_cos_sin: typing.Tuple[torch.Tensor, torch.Tensor]):
  ...
  with torch.amp.autocast('cuda', enabled=False):
    ...
    if flash_attn is None:
      qkv_rotary = apply_rotary_pos_emb_torchscript(
        torch.cat([q.unsqueeze(2), k.unsqueeze(2), v.unsqueeze(2)], dim=2),
        cos,
        sin,
      )
      return qkv_rotary[:, :, 0], qkv_rotary[:, :, 1], qkv_rotary[:, :, 2]
    if cos.shape[0] == 1:
    ...
    q = flash_attn.layers.rotary.apply_rotary_emb_torch(q, cos_in, sin_in)
    k = flash_attn.layers.rotary.apply_rotary_emb_torch(k, cos_in, sin_in)
  return q, k, v
...
def apply_rotary_pos_emb_single(vec: torch.Tensor, cos: torch.Tensor, sin: torch.Tensor):
  ...
  with torch.amp.autocast('cuda', enabled=False):
    ...
    if flash_attn is None:
      cos_single = cos[:, :, 0, 0, :]
      sin_single = sin[:, :, 0, 0, :]
      return (vec * cos_single) + (rotate_half(vec) * sin_single)
    if cos.shape[0] == 1:
      cos_in = cos[0, :, 0, 0, :cos.shape[-1] // 2]
      sin_in = sin[0, :, 0, 0, :sin.shape[-1] // 2]
    else:
...
class DDiTFinalLayer(nn.Module):
  ...
  def forward(self, x: torch.Tensor, c: typing.Optional[torch.Tensor]):
    ...
    if self.adaLN and c is not None:
      ...
      if c.ndim == 3:  # [B, n, cond_dim] - per-token
        shift, scale = self.adaLN_modulation(c).chunk(2, dim=2)  # Each [B, n, dim]
      elif c.shape[0] == x.shape[0]:  # [B, cond_dim] - global, broadcast
        shift, scale = self.adaLN_modulation(c)[:, None].chunk(2, dim=2)  # Each [B, 1, dim]
      else:
...
class DDiTBlockCausal(nn.Module):
  ...
  def _apply_causal_attention(self, qkv, rotary_cos_sin):
    ...
    if self.attn_backend == 'flash_attn' or (self.attn_backend == 'auto' and supports_flash_attention()):
      ...
      # Squeeze out the dimension created by chunking: [B, S, 1, H, D] -> [B, S, H, D]
...
__all__ = [
  ...
  'flash_varlen_attention_qkvpacked', 'flash_cross_attention',
]
...
def sdpa_attention(
  ...
  k = k.transpose(1, 2)
  v = v.transpose(1, 2)
  # CRITICAL: Cast mask to q.dtype for dtype hygiene (bf16/fp16/fp32 consistency)
  if attn_mask is not None:
        # IMPORTANT: project-wide convention seems to be:
    #   True = allowed, False = blocked
    if attn_mask.dtype == torch.bool:
      neg = torch.finfo(q.dtype).min  # or a large negative like -1e4
      attn_mask = torch.where(attn_mask, torch.zeros((), device=q.device, dtype=q.dtype), neg)
    else:
      attn_mask = attn_mask.to(dtype=q.dtype)

    attn_mask = attn_mask[:, None]  # [B, 1, S, S]
...
def flash_cross_attention(
  ...
  """FlashAttention for cross-attention with separate q, k, v.

  Args:
    q: Query tensor shaped (B, Sq, H, D)
    k: Key tensor shaped (B, Sk, H, D)
    v: Value tensor shaped (B, Sk, H, D)
    causal: Whether to apply causal masking.
    dropout_p: Dropout probability.

  Returns:
    Tensor shaped (B, Sq, H*D)
  """
  if flash_attn is None:
    raise RuntimeError("flash_attn is not available for flash_cross_attention")
  # flash_attn_func expects (B, S, H, D) and returns (B, S, H, D)
  x = flash_attn.flash_attn_func(q, k, v, dropout_p=dropout_p, causal=causal)
  return rearrange(x, 'b s h d -> b s (h d)')
