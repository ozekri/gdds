...
def _default_num_proc() -> int:
    """Best-effort cross-platform worker count."""
    try:
...
def wrap_tokenizer_decode_methods(tokenizer):
    """Wrap tokenizer decode and batch_decode methods to always include clean_up_tokenization_spaces=False.
    
    This ensures that even internal transformers library calls will use the parameter,
    preventing FutureWarning about clean_up_tokenization_spaces.
    ...
    if hasattr(tokenizer, 'decode'):
        ...
        if hasattr(tokenizer_class, 'decode'):
            # Get the function from the method descriptor
            original_decode_func = tokenizer_class.decode
            if hasattr(original_decode_func, '__func__'):
                original_decode_func = original_decode_func.__func__
        else:
            # Fallback: use bound method
            ...
            tokenizer.decode = types.MethodType(wrapped_decode, tokenizer)
            original_decode_func = None
        
        if original_decode_func is not None:
            # Create wrapper that properly handles self
            def wrapped_decode(self, *args, **kwargs):
                if 'clean_up_tokenization_spaces' not in kwargs:
                    kwargs['clean_up_tokenization_spaces'] = False
                return original_decode_func(self, *args, **kwargs)
            
            tokenizer.decode = types.MethodType(wrapped_decode, tokenizer)
    
    # Wrap batch_decode method
    if hasattr(tokenizer, 'batch_decode'):
        # Get the original unbound method from the class
        tokenizer_class = type(tokenizer)
        if hasattr(tokenizer_class, 'batch_decode'):
            # Get the function from the method descriptor
            original_batch_decode_func = tokenizer_class.batch_decode
            if hasattr(original_batch_decode_func, '__func__'):
                original_batch_decode_func = original_batch_decode_func.__func__
        else:
            # Fallback: use bound method
            ...
            tokenizer.batch_decode = types.MethodType(wrapped_batch_decode, tokenizer)
            original_batch_decode_func = None
        
        if original_batch_decode_func is not None:
            # Create wrapper that properly handles self
            def wrapped_batch_decode(self, *args, **kwargs):
                if 'clean_up_tokenization_spaces' not in kwargs:
                    kwargs['clean_up_tokenization_spaces'] = False
                return original_batch_decode_func(self, *args, **kwargs)
...
def get_tokenizer(config):
  if config.data.tokenizer_name_or_path == "text8":
    ...
    tokenizer = transformers.BertTokenizer.from_pretrained(
      "bert-base-uncased")
    # Wrap immediately after creation
    ...
    # Wrap immediately after creation, before any other operations
    wrap_tokenizer_decode_methods(tokenizer)
  
  if (
      isinstance(tokenizer, (transformers.GPT2TokenizerFast, transformers.GPT2Tokenizer))
      and hasattr(tokenizer, "_tokenizer")
      and getattr(tokenizer, "_tokenizer", None) is not None
  ):
  ...
  # Wrap again after all modifications to ensure it's still wrapped
  # (in case add_special_tokens or other operations replaced the methods)
  wrap_tokenizer_decode_methods(tokenizer)
