#!/usr/bin/env python3
"""Visualize SIK forward process jump-by-jump with the KeOps kernel at t=1.0."""
...
from src.discrete_diffusion.forward_process import SIKForwardProcess, KeOpsKernel
from src.discrete_diffusion.noise_schedules import LogLinear
from src.discrete_diffusion.data import get_tokenizer


def main():
    parser = argparse.ArgumentParser(description="Visualize SIK noising with the KeOps kernel.")
    ...
    print(f"[*] Building SIK KeOps kernel ({args.metric})...")
    kernel = KeOpsKernel(
    ...
    print("\n" + "=" * 72)
    print("Sequence Evolution (SIK + KeOps Kernel)")
    print("=" * 72)
    ...
    print("[*] Note: this static visualizer does not color-code transitions.")
